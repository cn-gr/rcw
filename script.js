document.addEventListener("DOMContentLoaded",function(){let e="",t=[],_=[],i=0,l=0,n,r=document.createElement("div");r.id="rcw-container",document.body.appendChild(r),r.innerHTML=`
    ...
    `;let s=document.getElementById("rcw-modal-body"),a=document.getElementById("rcw-modal-body-call"),c=document.getElementById("rcw-modal-body-result"),d=document.getElementById("rcw-input"),$=document.getElementsByClassName("rcw-choose-date")[0],o=document.getElementsByClassName("rcw-choose-date-now")[0],w=document.getElementsByClassName("rcw-select-call-date")[0],p=document.getElementById("rcw-modal"),v=document.getElementById("rcw-phone"),y=document.getElementById("rcw-button-submit"),g=document.getElementsByClassName("rcw-close")[0],m=document.getElementsByClassName("rcw-body-title")[0],u=document.getElementsByClassName("rcw-body-title-sub")[0],h=async()=>{let t=new FormData;t.append("site",window.location.origin);let _=await fetch("https://citynet.ge/b2b/rcw/get_api_key.php",{method:"POST",body:t});if(_.ok)try{let i=await _.text();(e=i)?.length>0&&(v.style.display="block")}catch(l){}};h(),v.onclick=function(){o.click(),s.classList.remove("result-body"),p.style.display="block",a.style.display="block",c.style.display="none",document.getElementById("rcw-error-message-date").textContent="",E(1),y.removeAttribute("disabled"),y.classList.remove("loading"),e&&f()},g.onclick=function(){p.style.display="none",d.value=""},window.onclick=function(e){e.target==p&&(p.style.display="none",d.value="")},$.onclick=function(){_=M(t),x(),_=B(_),document.getElementById("rcw-day-pick").classList.remove("rcw-error-select-message"),document.getElementById("rcw-time-pick").classList.remove("rcw-error-select-message");let e=_[0]?.value;T("rcw-day-pick",_.map(e=>({value:e.value,label:e.date}))),T("rcw-time-pick",C(e)),w.style.setProperty("display","flex"),$.style.setProperty("display","none"),o.style.setProperty("display","block"),i=1},o.onclick=function(){w.style.removeProperty("display"),o.style.setProperty("display","none"),$.style.setProperty("display","block"),i=0};let k=document.getElementById("rcw-keypad");k.addEventListener("click",function(e){let t=e.target;if(t.classList.contains("rcw-key")){let _=d.value,i=t.innerText;"←"===i?d.value=_.slice(0,-1):"x"===i?d.value="":d.value=_+i,b()}}),d.addEventListener("input",function(){b()}),y.addEventListener("click",function(e){b()&&L()});let b=()=>{let e=document.getElementById("rcw-error-message");return/^\+?(995)?0?(5\d{8}|(32)?2\d{6}|7\d{8}|(3|4)\d{2}\d{6})$/.test(d.value)?(e.textContent="",!0):(e.textContent="ნომერი არასწორად არის მითითებული",!1)},f=async()=>{let i=new FormData;i.append("apikey",e),i.append("site",window.location.origin);let l=await fetch("https://citynet.ge/b2b/rcw/get_schedule.php",{method:"POST",body:i});if(l.ok)try{let n=await l.json();t=n,_=M(t),x()}catch(r){}},E=(e,t=[])=>{1==e?(m.innerHTML="გნებავთ ჩვენ გადმოგირეკავთ?",u.innerHTML="მიუთითეთ თქვენი ნომერი და ჩვენ დაგიკავშირდებით 1 წუთის განმავლობაში!"):2==e?(m.innerHTML="სამუშაო საათები დასრულებულია",u.innerHTML="დაგირეკავთ შერჩეულ დროს"):3==e?(m.innerHTML="დღეს ვისვენებთ",u.innerHTML="დაგიკავშირდებით შერჩეულ დროს"):4==e?(m.innerHTML=`შესვენება ${t[0]}-დან ${t[1]}-მდე`,u.innerHTML="დაგიკავშირდებით შერჩეულ დროს"):5==e&&(m.innerHTML=`ახლა არასამუშაო დროა`,u.innerHTML="დაგიკავშირდებით შერჩეულ დროს")},x=()=>{let e=_.some(e=>e.value===H().substring(0,10)),t=_[0]?.working_hours.length>0;if(!e||!t){t?E(3):E(2),$.click(),o.style.setProperty("display","none");return}let i=function e(t){let _=new Date;for(let i of t){let l=new Date(i.value+" "+i.work_start),n=new Date(i.value+" "+i.work_end);if(_>=l&&_<=n){if(null===i.break_start||null===i.break_end)return!0;{let r=new Date(i.value+" "+i.break_start),s=new Date(i.value+" "+i.break_end);if(_<r||_>s)return!0}}}return!1}(_),l=new Date().getTime();void 0!=_[0].break_start&&void 0!=_.break_end?!i&&_[0].break_start<=l&&l<=_[0].break_end&&(E(4,[_[0].break_start.substring(0,5),_[0].break_end.substring(0,5)]),$.click(),o.style.setProperty("display","none")):i||(E(5),$.click(),o.style.setProperty("display","none"))},M=e=>Object.values(e).map(e=>{let t=new Date().getFullYear(),_=new Date().getMonth()+1,i=new Date().getDate(),l=new Date().getDay(),n=e.week_day_id-(0===l?7:l);n<0&&(n+=7);let r=new Date(t,_-1,i+n),s=`${r.getDate().toString().padStart(2,"0")}/${(r.getMonth()+1).toString().padStart(2,"0")}`,a=`${r.getFullYear()}-${(r.getMonth()+1).toString().padStart(2,"0")}-${r.getDate().toString().padStart(2,"0")}`,c=new Date,d=new Date(c);return d.setDate(c.getDate()+1),r.toDateString()===c.toDateString()?s="დღეს":r.toDateString()===d.toDateString()&&(s="ხვალ"),{...e,date:s,value:a,working_hours:S(e)}}).sort((e,t)=>{let _=new Date(e.value),i=new Date(t.value);return _-i}),B=e=>e.filter(e=>e.working_hours.length>0),L=async()=>{let t="";if(0==i)t=H(new Date);else{let _=document.getElementById("rcw-day-pick").value,r=document.getElementById("rcw-time-pick").value;t=H(`${_} ${r}:00`)}let $=new FormData;$.append("apikey",e),$.append("phone",d.value),$.append("time",t),$.append("site",window.location.origin),0==i&&$.append("immediately",1);let o=await fetch("https://citynet.ge/b2b/rcw/request_call.php",{method:"POST",body:$});if(o.ok)try{var w;let p=await o.json(),v;if(("success"==p.result||"exists"==p.error)&&(a.style.display="none",c.style.display="block",w=t.replace("T"," "),s.classList.add("result-body"),v="",0==i?(v=`<div style="text-align: center;">
                  ...
                  </div>`,clearInterval(n),l=6e4,D(),n=setInterval(D,100)):v=`<div style="text-align: center;">
                      <h2>მადლობა</h2>
                      <h4>ჩვენ დაგიკავშირდებით! დრო: ${w}</h4>
                    </div>`,c.innerHTML=v),"passed date"==p.error){let y=document.getElementById("rcw-error-message-date");document.getElementById("rcw-day-pick").classList.add("rcw-error-select-message"),y.textContent="მიუთითეთ ვალიდური თარიღი"}if("passed time"==p.error){let g=document.getElementById("rcw-error-message-date");document.getElementById("rcw-time-pick").classList.add("rcw-error-select-message"),g.textContent="მიუთითეთ ვალიდური დრო"}"spam"==p.error&&(c.innerHTML='<div class="rcw-loading"></div>',a.style.display="none",c.style.display="block")}catch(m){}};function H(e=new Date,t=0){let _=new Date(e);_.setSeconds(_.getSeconds()+t);let i=_.getFullYear(),l=(_.getMonth()+1).toString().padStart(2,"0"),n=_.getDate().toString().padStart(2,"0"),r=_.getHours().toString().padStart(2,"0"),s=_.getMinutes().toString().padStart(2,"0"),a;return i+"-"+l+"-"+n+"T"+r+":"+s+":"+_.getSeconds().toString().padStart(2,"0")}function I(e){let[t,_]=e.split("T"),[i,l,n]=t.split("-"),[r,s,a]=_.split(":");return new Date(i,l-1,n,r,s,a)}let S=e=>{let t=[],_=new Date().getFullYear(),i=new Date().getMonth()+1,l=(new Date().getDay()+6)%7+1,n=parseInt(e.week_day_id,10),r=new Date().getHours(),s=new Date().getMinutes(),a=I(`${_}-${i}-${l}T${e.work_start}`),c=I(`${_}-${i}-${l}T${e.work_end}`),d=e.break_start?I(`${_}-${i}-${l}T${e.break_start}`):null,$=e.break_end?I(`${_}-${i}-${l}T${e.break_end}`):null,o=new Date(a);for(l===n&&r>o.getHours()&&o.setHours(r,15*Math.ceil(s/15),0);o<c;){if(!(d&&o>=d&&o<=$)&&o>a){let w=o.toLocaleTimeString("ka-GE",{hour12:!1}).slice(0,5);l===n&&o.getMinutes()===s||t.push({value:w,label:w})}o.setMinutes(o.getMinutes()+15)}return t};function T(e,t){let _=document.getElementById(e);_.innerHTML="",t.forEach(e=>{let t=document.createElement("option");t.value=e.value,t.textContent=e.label,_.appendChild(t)})}function C(e){let t=_.find(t=>t.value===e);return t?t.working_hours:[]}function D(){let e=Math.floor(l/1e3),t=Math.floor(l%1e3/10),_=`${e<10?"0":""}${e}.${t<10?"0":""}${t}`,i=document.getElementById("rcw-timer");i&&(i.textContent=_,(l-=100)<0&&(clearInterval(n),c.innerHTML=`<div style="text-align: center;">
                                            <div>
